...
Batch Configurations define how and where UniServe 360 connects when running batch jobs. \
It controls server access, database connectivity, and processing behavior for large-volume document generation. Set this up once per environment (QA, UAT, production, or client-specific) and reuse it across batch processes.

Batch configurations include server (to run batch jobs reliably, securely, and at scale) and database configurations (to fetch data).

To set up batch configurations, navigate to **Settings** > **Batch Configurations**
